        ...
        monitorThread = new Thread(new Runnable() {
            ...
            public void run() {
                    ...
                    try {
                        ...
                        if (CollectionUtils.isNotEmpty(jobLogIdList)) {
                            for (Integer jobLogId : jobLogIdList) {
                                ...
                                if (ApiResult.SUCCESS_CODE == log.getTriggerCode() && log.getHandleCode() == 0) {
                                    JobFailMonitorHelper.monitor(jobLogId);
                                    logger.info(">>>>>>>>>>> job monitor, job running, JobLogId:{}", jobLogId);
                                } else if (ApiResult.SUCCESS_CODE == log.getTriggerCode()
                                        && ApiResult.SUCCESS_CODE == log.getHandleCode()) {
                                    // job success, pass
                                    logger.info(">>>>>>>>>>> job monitor, job success, JobLogId:{}", jobLogId);
                                } else if (ApiResult.FAIL_CODE == log.getTriggerCode()
                                        || ApiResult.FAIL_CODE == log.getHandleCode()) {
                                    // job fail,
                                    failAlarm(log);
                ...
                if (jobLogIdList != null && jobLogIdList.size() > 0) {
                    for (Integer jobLogId : jobLogIdList) {
                        XxlJobLog log = XxlJobDynamicScheduler.xxlJobLogDao.load(jobLogId);
                        if (ApiResult.FAIL_CODE == log.getTriggerCode() || ApiResult.FAIL_CODE == log.getHandleCode()) {
                            // job fail,
                            failAlarm(log);
        ...
        if (info != null && info.getAlarmEmail() != null && info.getAlarmEmail().trim().length() > 0) {
            ...
            for (String email : emailSet) {
                ...
                String content = MessageFormat.format("任务调度失败, 执行器名称:{0}, 任务描述:{1}.",
                        group != null ? group.getTitle() : "null", info.getJobDesc());
