...
public class AdminBizImpl implements AdminBiz {
    ...
    public ApiResult<String> callback(List<HandleCallbackParam> callbackParamList) {
        for (HandleCallbackParam handleCallbackParam : callbackParamList) {
            ...
            logger.info("JobApiController.callback {}, handleCallbackParam={}, callbackResult={}",
                    (callbackResult.getCode() == ApiResult.SUCCESS_CODE ? "success" : "fail"), handleCallbackParam,
                    callbackResult);
        }
    ...
    private ApiResult<String> callback(HandleCallbackParam handleCallbackParam) {
        ...
        if (log == null) {
            return new ApiResult<String>(ApiResult.FAIL_CODE, "log item not found.");
        }

        // trigger success, to trigger child job, and avoid repeat trigger child job
        String childTriggerMsg = null;
        if (ApiResult.SUCCESS_CODE == handleCallbackParam.getExecuteResult().getCode()
                && ApiResult.SUCCESS_CODE != log.getHandleCode()) {
            ...
            if (xxlJobInfo != null && StringUtils.isNotBlank(xxlJobInfo.getChildJobKey())) {
                ...
                for (int i = 0; i < childJobKeys.length; i++) {
                    ...
                    if (jobKeyArr != null && jobKeyArr.length == 2) {
                        ApiResult<String> triggerChildResult = xxlJobService.triggerJob(Integer.valueOf(jobKeyArr[1]));
                        // add msg
                        childTriggerMsg += MessageFormat.format("<br> {0}/{1} 触发子任务{2}, 子任务Key: {3}, 子任务触发备注: {4}",
                                (i + 1), childJobKeys.length,
                                (triggerChildResult.getCode() == ApiResult.SUCCESS_CODE ? "成功" : "失败"), childJobKeys[i],
